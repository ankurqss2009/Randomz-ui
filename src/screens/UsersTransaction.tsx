  ...
  const { data, error, isLoading, isPreviousData } = useQuery(
    ...
    async () => {
      const { data } = await axios.get<UsersTransactions>(
        `${process.env.VITE_SERVER_URL}/api/transactions/getAllTransactions?page=${page}`,
        {
          headers: {
    ...
    <div className="md:border border-[#303030] w-full min-h-screen p-4 md:px-8 md:py-0">
      <div className="z-10 relative overflow-x-auto w-full  p-4 transactiontable my-10 shadow-sm shadow-[#172147]">
        {isLoading ? (
          ...
          <>
            {(data as UsersTransactions)?.results?.length > 0 ? (
              <>
                <table className="w-full text-sm text-left text-white ">
                  ...
                  <tbody>
                    {!isLoading &&
                      (data as UsersTransactions)?.results?.length > 0 &&
                      (data as UsersTransactions).results?.map(
                        (transaction, index: number) => {
                          return (
                            ...
                            >
                              ...
                              <td className="px-6 py-4 ">
                                {transaction?.user?.email ? transaction?.user?.email : "Not available"}
                              </td>
                              <td className="px-6 py-4">{transaction?.user?.referedBy}</td>
                              <td className="px-6 py-4">
                                <Link
                                  target="_blank"
