...
const TransactionTable = ({data, error=null, isLoading=false, isPreviousData=false}) => {
  ...
  /*const { data, error, isLoading, isPreviousData } = useQuery(
    ...
    async () => {
      const { data } = await axios.get<User_Personal_Transactions>(
        `${process.env.VITE_SERVER_URL}/api/transations/?page=${page}`,
        {
          headers: {
  ...
  return (
    <div className="z-10 relative overflow-x-auto w-full max-w-3xl  p-4 transactiontable my-10 shadow-sm shadow-[#172147]">
      {isLoading ? (
        ...
        <>
          {(data as User_Personal_Transactions)?.length > 0 ? (
            <>
              <table className="w-full text-sm text-left text-white ">
                ...
                <tbody>
                  {!isLoading &&
                    (data as User_Personal_Transactions)?.length >
                      0 &&
                    (data as User_Personal_Transactions).map(
