  ...
  const { data, error, isLoading, isPreviousData } = useQuery(
    ...
    async () => {
      const { data } = await axios.get<UsersClaimRequest>(
        `${process.env.VITE_SERVER_URL}/api/claimRequest/getAll?page=${page}`,
        {
          headers: {
    ...
    <div className="md:border border-[#303030] w-full min-h-screen p-4 md:px-8 md:py-0">
      <div className="z-10 relative overflow-x-auto w-full  p-4 transactiontable my-10 shadow-sm shadow-[#172147]">
        {isLoading ? (
          ...
          <>
            {(data as UsersClaimRequest | undefined)?.results.length === 0 ? (
              ...
              <>
                <table className="w-full text-sm text-left text-white ">
                  ...
                  <tbody>
                    {!isLoading &&
                      (data as UsersClaimRequest)?.results.length > 0 &&
                      (data as UsersClaimRequest).results.map(
                        (claim, index: number) => {
                          return (
                            <tr
                              key={index}
                            ...
                            >
                              ...
                              <th className="px-6 py-4">{claim.email}</th>
                              <td className="px-6 py-4 ">
                                {convertToDateString(claim.createdAt)}
                              </td>
                              <td className="px-6 py-4">
                              ...
                              <td className="px-6 py-4">
                                {claim.txid ? (
                                  <Link
                                    ...
                                    target="_blank"
                                    to={`https://testnet.bscscan.com/tx/${claim.txid}`}
                                  >
                                    <p>{claim.txid.slice(0, 16)}</p>
                                    <svg
                ...
                >
                  <ul className="inline-flex items-center -space-x-px">
                    ...
                    <div className="flex gap-2 px-3">
                      {data?.results &&
                        [...Array(Math.ceil(data?.totalCount / PAGE_SIZE))].map(
                          (_, index) => {
                            return (
                              <li key={index + 1}>
                                <button
